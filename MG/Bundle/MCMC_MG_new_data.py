...
def log_posterior(params, a_data, fs8_data, fs8_err):
    Om_m_0, s8, g_a  = params
    if 0.05 < Om_m_0 < 0.7 and 0.5 < s8 < 1.3 and -1.5 < g_a < 0 :
...
'''
backend   = emcee.backends.HDFBackend('chain_nn_new_MG.h5')
...
fig.tight_layout()
fig.savefig('chains_nn_new_MG.png')
plt.show()
...
fig = corner.corner( samples, labels=[ "$\Omega_{m0}$", "$\sigma_8$", "$g_a$"], truths=[init0, init1, init2], \
                              ...
                              scale_hist=True )
plt.show()
fig.savefig('triangplot_nn_new_MG.png')
plt.close()
