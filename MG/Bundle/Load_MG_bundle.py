...
def x(N, Om_m_0, g_a):
    ...
    g_a_vec = g_a * np.ones_like(N)
    xs = sol(N, Om_m_vec, g_a_vec, to_numpy=True)[0]
    return xs

def y(N, Om_m_0, g_a):
    Om_m_vec = Om_m_0 * np.ones_like(N)
    g_a_vec = g_a * np.ones_like(N)
    ys = sol(N, Om_m_vec, g_a_vec, to_numpy=True)[1]
    return ys
...
x_nn=x(N_vec, Om_m, g_a)
y_nn=y(N_vec, Om_m, g_a)
