...
class EtaEstimation:
    def __init__(self, v_sol, v_index, Om_m_0, res_fun, f_fun, t_min, t_max, N_for_int=int(1e3), j_max=6):
        """
    ...
    def C(self, t):
        ...
        v_sol_val = self.v_sol(t, Om_t, to_numpy=True)
    ...
    def compute_etas(self):
        ...
        if self.multiple_dependent_variables:
            ress = self.res(*self.v_sol(ts_diff, Om_t), ts_diff, self.Om_m_0)[self.v_index].reshape(1, -1).detach().numpy()[0]
        else:
