...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 21 16:08:50 2024
...
def make_bound_g(X,Y,B_X,B_Y): # The bound in \delta.
    return np.sqrt((X*B_Y)**2+(Y*B_X)**2)

def make_bound_g_TVM(X,Y,B_X,B_Y):
    bound_grad = []
    ...
    return np.sqrt(2)*bound_grad*np.sqrt(B_X**2+B_Y**2)
...
plt.xscale('log')
plt.savefig('heatmap_fs8_LCDM.pdf')
plt.show()
...
plt.ylabel(r'$\Omega_{m0}$')
plt.xscale('log')
plt.savefig('heatmap_fs8_LCDM_num_diff.pdf')
plt.show()
...
plt.xlabel(r'$a$')
plt.ylabel(r'$\Omega_{m0}$')
plt.savefig('res_fs8_LCDM.pdf')
plt.title(str(np.min(res_fs8)))
plt.show()
#%%
res_x = np.array(b_x) - np.array(diff_x)
res_y = np.array(b_y) - np.array(diff_y)    

res_d = np.array(b_d) - np.array(diff_d)
res_dp = np.array(b_dp) - np.array(diff_dp) 

plt.figure()
plt.pcolormesh(ts, Om_m_0s, res_x, cmap='viridis')  # 'viridis' es un mapa de colores, puedes elegir otro
plt.colorbar()
plt.xlabel(r'$a$')
plt.ylabel(r'$\Omega_{m0}$')
plt.title(str(np.min(res_x)))
#plt.savefig('heatmap_fs8.png')
plt.show()

plt.figure()
plt.pcolormesh(ts, Om_m_0s, res_y, cmap='viridis')  # 'viridis' es un mapa de colores, puedes elegir otro
plt.colorbar()
plt.xlabel(r'$a$')
plt.ylabel(r'$\Omega_{m0}$')
plt.title(str(np.min(res_y)))
#plt.savefig('heatmap_fs8.png')
plt.show()
...
'''
plt.figure()
plt.pcolormesh(np.exp(n_i*ts), Om_m_0s, b_x, cmap='viridis')  # 'viridis' es un mapa de colores, puedes elegir otro
plt.colorbar()
plt.xlabel(r'$a$')
plt.xlim(np.min(a_dat),np.max(a_dat))
plt.ylabel(r'$\Omega_{m0}$')
...
plt.figure()
plt.pcolormesh(np.exp(n_i*ts), Om_m_0s, diff_x, cmap='viridis')  # 'viridis' es un mapa de colores, puedes elegir otro
plt.colorbar()
plt.xlabel(r'$a$')
plt.xlim(np.min(a_dat),np.max(a_dat))
plt.ylabel(r'$\Omega_{m0}$')
plt.title(r'%err $f\sigma_8$ (numerical)')
#plt.savefig('heatmap_fs8.png')
plt.show()

#%%
residual = np.array(b_fs8_per) - np.array(diff_fs8)

plt.figure()
plt.pcolormesh(np.exp(n_i*ts), Om_m_0s, residual, cmap='viridis')  # 'viridis' es un mapa de colores, puedes elegir otro
plt.colorbar()
plt.xlabel(r'$a$')
plt.xlim(np.min(a_dat),np.max(a_dat))
plt.ylabel(r'$\Omega_{m0}$')
plt.title(r'bound% - err%')
#plt.savefig('heatmap_fs8.png')
plt.show()
