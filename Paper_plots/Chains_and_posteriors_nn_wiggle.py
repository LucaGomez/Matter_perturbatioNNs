...
cov_matrix_inv[3:6,3:6] += non_diag_values
for i in range(6,20):
    cov_matrix_inv[i][i] += 1/err[i]**2
...
plt.legend()
plt.savefig('posteriors_LCDM_no28.pdf')
plt.show()
...
print(best_fit_params_LCDM)
print(mean_LCDM_pars)
...
plt.legend()
plt.savefig('Data_comp_LCDM_no28.pdf')
plt.show()
...
def log_posterior(params, a_data, fs8_data, fs8_err):
    Om_m_0, s8, ga  = params
    if 0.05 < Om_m_0 < 0.7 and 0.5 < s8 < 1.5 and -3e-2 < ga < 3e-2:
        logpost = log_likelihood(params, a_data, fs8_data, fs8_err)
    else:
        logpost = -np.inf
    return logpost
...
print(best_fit_params_MG)
print(red_chi_MG_bfit)
...
# Graficar fs8 vs z
plt.errorbar(z,fs8_data, yerr = err, fmt ='o', label = 'Data')
plt.plot(z_eval,fs8_bfit_LCDM, label = r'$\Lambda$CDM')
plt.plot(z_eval,fs8_bfit_MG, label = r'MG')
ax1.set_xlabel('$z$')
ax1.set_ylabel('$f\sigma_8$')
plt.legend()
ax1.grid()

z_twin = np.linspace(z[0],z[-1],8)
# Crear un segundo eje x en la parte superior
ax2 = ax1.twiny()

# Sincronizar los límites del eje superior con los del eje inferior
ax2.set_xlim(ax1.get_xlim())
ax2.set_xticks(z_twin)  # Elegir valores representativos de z
ax2.set_xticklabels([f'{val:.2f}' for val in 1 / (1 + z_twin)])
ax2.set_xlabel('$a$')
...
plt.legend()
plt.tight_layout()
plt.savefig('Comp_LCDM_MG_data_zva.pdf')
plt.show()
...
ndim1  = len(names1)
samples11 = MCSamples(samples=flat_samples1, names=names1, labels=labels1)
samples11 = samples11.copy(settings={'mult_bias_correction_order':0,'smooth_scale_2D':0.1,
					'smooth_scale_1D':0.1})
...
plt.savefig('posteriors_LCDM_MG.pdf')
plt.show()